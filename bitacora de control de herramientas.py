...
# Lee usuario y contraseña desde los secrets de Streamlit Cloud
# Si no existen, usa valores por defecto (solo para desarrollo local)
USUARIO_VALIDO = st.secrets.get("USERNAME", "ACI")
CONTRASENA_VALIDA = st.secrets.get("PASSWORD", "ACI940930L91")
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        c.execute("INSERT INTO usuarios (nombre) VALUES (?)", ("EMV",))
    conn.commit()
    conn.close()
